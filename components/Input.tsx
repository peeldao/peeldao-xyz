...
export function Input(props: React.HTMLProps<HTMLInputElement>) {
  const inputClass = `focus:ring-cyan-500 py-3 focus:border-cyan-500 block w-full pl-7 text-md border border-slate-500 rounded-md bg-transparent text-white     ${
    ...
    <div>
      ...
      <div className="relative rounded-md shadow-sm">
        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
          <span className="text-gray-200 sm:text-sm font-thin">
            {ETH_SYMBOL}
          </span>
        </div>
        ...
        <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
          <span
            className="text-gray-200 sm:text-sm font-thin"
            id="amount-currency"
          >
            ETH
          </span>
